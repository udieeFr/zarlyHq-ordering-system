...
## 10. Email OTP — Cache-Based Storage

**File:** `customers/views.py`
**Functions:** `generate_and_cache_otp`, `verify_otp`, `resend_verification_otp`
**Used by:** `verify_email_submit`, `verification_required`

### Why Cache Instead of a Database Column

The instinct is to store a one-time code in a database column (e.g. `otp_code VARCHAR`, `otp_expires_at DATETIME`). This works, but introduces problems:

- **Cleanup burden.** Expired codes accumulate as dead rows unless a cron job or post-verify cleanup deletes them.
- **Schema coupling.** The OTP lifecycle is tied to the User model, making migrations necessary for what is really ephemeral state.
- **Timing leak risk.** A DB column persists the code even after use unless explicitly nulled — a window where a leaked DB backup could reveal recent codes.

The cache solves all three. The code is stored with a TTL and the cache layer expires it automatically. On correct submission the entry is deleted immediately (single-use). If the cache restarts, old codes are gone — users just request a new one.
...
CACHE_KEY   = 'email_otp_{user_pk}'
OTP_TTL     = 300  # seconds — 5 minutes

def generate_and_cache_otp(user):
    code = f"{secrets.randbelow(1_000_000):06d}"   # 000000–999999
    cache.set(CACHE_KEY.format(user_pk=user.pk), code, OTP_TTL)
    return code

def verify_otp(user, submitted_code):
    key    = CACHE_KEY.format(user_pk=user.pk)
    stored = cache.get(key)
    if stored is None:
        return 'expired'          # TTL elapsed or never sent
    if stored != submitted_code:
        return 'invalid'          # wrong code
    cache.delete(key)             # single-use: delete immediately
    return 'ok'
```
...
|---|---|
| Cryptographic randomness | `secrets.randbelow` (not `random`) — uses OS entropy |
| Short lived | 5-minute TTL set at cache write time — enforced by cache layer, not app logic |
| Single use | Cache key deleted on first correct submission |
| Resend invalidates old code | `cache.set` overwrites the key — previous code unreachable |
| Brute-force resistance | `verify_email_submit` is rate-limited: 10 attempts / 5 minutes per user |

### Attack: Brute-Force a 6-Digit Code

A 6-digit numeric code has 1,000,000 possible values. At 10 attempts per 5 minutes,
an attacker needs on average 500,000 attempts to guess it — that is ~41,666 hours
(over 4 years) at the enforced rate. The 5-minute TTL expires the code long before
any meaningful number of guesses can be made.

### Cache Backend Note

Development uses `LocMemCache` (single-process, in-memory). This is fine locally but
does not survive server restarts and does not work across multiple worker processes.
...
Rate limiting (django-ratelimit) also uses the cache backend — Redis covers both.

### How to Test

1. Register a new account at `/register/`.
2. On the verify page, check your email for the 6-digit code.
3. Enter a wrong code — you should see "Incorrect or expired code."
4. Enter the correct code — account marked verified, redirected to menu.
5. Try reusing the same code — it should say "expired" (single-use confirmed).
6. Click "Resend" and verify the new code works while the old one no longer does.
